 ...
 * 
 * This class is a singleton class: since all empty search trees are the same, there is no need for
 * multiple instances of this class. Instead, a single instance of the class is created and made
 * available through the static field SINGLETON.
 * 
 * The constructor is private, preventing other code from mistakenly creating additional instances
 * of the class. */
public class EmptyTree<K extends Comparable<K>, V> implements Tree<K, V> {

    /** This static field references the one and only instance of this class. We won't declare
     * generic types for this one, so the same singleton can be used for any kind of EmptyTree. */
